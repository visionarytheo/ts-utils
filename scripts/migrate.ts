...
async function runMigration() {
  try {
    const migrationsDir = path.join(process.cwd(), "src", "utils", "db", "migrations");
    ...
    if (pendingFiles.length === 0) {
      console.log(
        "✨ Database is completely up to date. No pending migrations.",
      );
      return;
    }

    console.log(
      `⏳ Found ${pendingFiles.length} new migration files to apply.`,
    );
